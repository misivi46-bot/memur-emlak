...
st.markdown("""
    <style>
        ...
        /* 1. ÇÖZÜM: Sekme Menüsü ve İmleç (Caret) Yok Edici */
        div[role="radiogroup"] { 
            flex-direction: row; 
            gap: 15px; 
            background: white;
            ...
            margin-bottom: 20px;
            outline: none !important; /* Tıklama çizgisini kaldırır */
        }
...
def koordinati_adrese_cevir(lat, lon):
    try:
        loc = Nominatim(user_agent="memur_emlak_v25").reverse(f"{lat}, {lon}", timeout=3)
...
with st.sidebar:
    ...
    if secilen_sehir == "Türkiye Geneli":
        ...
        harita_zoom = 16 
        # Konum etrafındaki kurumları hesapla
...
# 1. ÇÖZÜM: Sekme geçişleri tek tıkla çalışsın diye kontrol eklendi
...
# Seçilen sekme hafızadakinden farklıysa hafızayı güncelle ve ANINDA render et
...
if aktif_sekme == "📍 Harita":
    ...
    # 2. ÇÖZÜM: Kurum ikonları (Mavi Çanta/Info) HER ZAMAN ve HER YERDE haritaya basılacak!
